...
![Сравнение моделей](task_9.png)

Метрики качества моделей оказались довольно схожими. Можель, соответствующая тематике помогает больше, чем более крупная общая модель, т.к. финансовая модель хоть и обучена на меньшем корпусе, но на более релевантном, поэтому по полученным метрикам качество почти идентично более крупной модели. Если её дообучить, то весьма вероятно, что метрики были бы сильно лучше.
